...
|loginCredentialsExtractionEnabled|Flag for that enables/disables the extraction of login credentials.|false|boolean
|loginCredentialsExtractionDetails|Maps the json login credentials configuration array to a dedicate class. The map helps in determine if incoming request is a login request or not.|null| CILoginMap
|ciProtocol|Credentials Intelligence protocol determines the strcuture and content of the user login data.|V2|CIProtocol
|pxCompromisedCredentialsHeader|The name of the header that is sent to the client to mark that account is breached.|"px-compromised-credentials"|String
...
|headerNameToValidateLoginResponse|Header name that uses to validate if the login is succesful.|x-px-login-successful|String
|headerValueToValidateLoginResponse|Header value that uses to validate if the login is successful.|"1"|String
|loginResponseValidationStatusCode|Array of status codes that uses to validate if the login was successful.|{200}|int[]
|customLoginResponseValidator|Custom class that validates if the login was successful.Must implement LoginResponseValidator.|DefaultCustomLoginResponseValidator|LoginResponseValidator
|credentialsCustomExtractor|Custom class that extracts the login credentials. Must implement CredentialsExtractor.|DefaultCredentialsCustomExtractor|CredentialsExtractor
